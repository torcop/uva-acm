...
int person, week, hotel , budget, person_cost ;

int weekend_capacity ,i,j,decision = 0;
...
REP(i,hotel){
    ...
    REP(j,week){
        ...
        scanf("%d", &weekend_capacity);
        vect.PB( weekend_capacity );

    }

    result = max_element(vect.begin(),vect.end());

    min_cost = person_cost * person;




            if( *result >= person &&  min_cost < budget){

                if( decision < 1 || decision > min_cost ){
